...
- `/empresa` cuenta de empresa (RUT + correo + clave), perfil, logo, CSV, liquidación y carta de finiquito
...
El esquema está en `sql/001.sql` (cuentas) y `sql/002.sql` (giro, dirección, clave de logo y documentos). Se aplica en el primer request si es seguro. `002` no borra cuentas existentes.

Opcional en el host, nunca en el repositorio:

- `DATABASE_URL` — conexión de la app
- `DATABASE_URL_UNPOOLED` — preferida para aplicar `sql/001.sql` y `sql/002.sql`
...
- `R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_BUCKET` — depósito privado para logos y PDF. Si faltan, `/api/logo` y `/api/documento` responden 501 (`no_storage`) y la interfaz lo dice; no se finge la subida.
