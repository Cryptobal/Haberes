...
Pro se cobra **$14.990 + IVA** (IVA 19 % Chile) = **17.838 CLP**, como **suscripción mensual automática**. `POST /api/checkout` acepta `{ provider: "mp" | "flow" }` (si se omite, Mercado Pago). `GET /api/checkout` y `/api/me` informan `providers: ["mp", "flow"]` según las claves presentes. Mercado Pago usa `preapproval` mensual. Flow usa plan + cliente + registro de tarjeta + `subscription/create`. Si el cobro se rechaza, anula o reembolsa, `plan` vuelve a `gratis`. El plan no se activa por `?pago=ok` en la URL: solo tras el webhook (`/preapproval` o `/v1/payments` en Mercado Pago; registro de tarjeta, `invoice/get` o `payment/getStatus` en Flow).

En el panel de Mercado Pago, apunte las notificaciones a `https://www.haberes.cl/api/mp-webhook`. En Flow, `urlCallback` del plan y `url_return` del registro de tarjeta usan `https://www.haberes.cl/api/flow-webhook`.
