...
```
node scripts/verify.mjs   # cálculo, contenido y contratos de API (419 aserciones)
...
- `Mp:access_token` (como está en Vercel), o `mp_access`, `MP_ACCESS_TOKEN`, `MERCADOPAGO_ACCESS_TOKEN`, `MP_ACCESS_TOKEN_PROD`, `MP_ACCESS` — token de Checkout Pro. Sin él, `POST /api/checkout` responde 501 (`mp_unavailable`) y el botón lo dice; no se finge el pago. No hace falta renombrar la variable.
