            ...
            : meta.calc === "aguinaldo"
              ...
              : meta.calc === "horas"
                ...
                : meta.calc === "casa-particular"
                  ...
                  : meta.calc === "feriado"
                    ? "Calcular vacaciones proporcionales — Haberes"
...
function calcLinkLabel(calc) {
  ...
  if (calc === "/trabajo-pesado") return "Calculadora trabajo pesado";
  ...
  if (calc === "/tutela-laboral") return "Calcular tutela laboral";
  if (calc === "/despido-injustificado") return "Calcular despido injustificado";
...
function hubList(items) {
  return `<ul>
        ${items
          .map((g) => {
            let extra = "";
            if (g.slug === "liquidacion-de-sueldo-y-previred") {
              extra = `\n          · <a href="/cotizaciones-previsionales">Calcular cotizaciones previsionales</a>\n          · <a href="/seguro-cesantia">Calcular seguro de cesantía</a>`;
            }
            ...
            if (g.slug === "indemnizacion-por-anos-de-servicio") {
              extra = `\n          · <a href="/finiquito">Calculadora de finiquito</a>\n          · <a href="/tutela-laboral">Calcular tutela laboral</a>\n          · <a href="/despido-injustificado">Calcular despido injustificado</a>`;
            }
...

{
  ...
  const hubBody = `
      ...
      <p>Qué es el comprobante de remuneraciones, descuentos legales e impuesto único. Para estimar el líquido use <a href="/sueldo">/sueldo</a>. Para el bruto de un mes incompleto (días calendario × remuneración / 30), <a href="/sueldo-proporcional">calcular sueldo proporcional</a>. Para el descuento bruto por atrasos e inasistencias injustificadas (DT /30), <a href="/descuento-atrasos">calcular descuento por atrasos e inasistencias</a>. Para el descuento por permiso sin goce de sueldo (pacto; no es permiso pagado ni atraso), <a href="/permiso-sin-goce">calcular permiso sin goce</a>. Para el bruto del mes con licencia médica y una estimación del SIL (D.F.L. N°44), <a href="/licencia-medica">calcular sueldo con licencia médica</a>. Para la retención judicial de pensión de alimentos (Ley 14.908 art. 8), <a href="/retencion-judicial">calcular retención judicial</a>. Para el efecto del APV Régimen B en el IUSC y el líquido del mes (art. 42 bis LIR), <a href="/apv">calcular APV Régimen B</a>. Para el piso legal del IMM (Ley 21.830), <a href="/sueldo-minimo">calcular sueldo mínimo</a>. Para AFP, salud y cesantía del trabajador, <a href="/cotizaciones-previsionales">calcular cotizaciones previsionales</a>. Para el tope imponible en UF y pesos del mes (90 UF AFP/salud, 135,2 UF cesantía) y cuánto de la renta queda sobre el tope, <a href="/tope-imponible">calcular tope imponible</a>. Para la retención de una boleta de honorarios (independientes, Ley 21.133), <a href="/boleta-honorarios">calcular retención boleta de honorarios</a>. Para el desglose AFC de trabajador y empleador, <a href="/seguro-cesantia">calcular seguro de cesantía</a>. Para la cotización adicional por trabajo pesado (Ley 19.404 / D.L. 3.500 art. 17 bis), <a href="/trabajo-pesado">calculadora trabajo pesado</a>. Para el monto por cargas acreditadas, <a href="/asignacion-familiar">calcular asignación familiar</a>. Para la obligación de sala cuna (art. 203, umbral 20 trabajadoras) y el costo si paga el establecimiento, <a href="/sala-cuna">calcular sala cuna</a>. Para la cuota del 1 % de inclusión laboral (Ley 21.015, umbral 100 trabajadores, gap y donación subsidiaria 24 IMM), <a href="/inclusion-laboral">calcular inclusión laboral</a>. Para el descanso prenatal de maternidad (art. 195: 6 semanas / 42 días corridos antes del parto), <a href="/permiso-prenatal">calcular permiso prenatal</a>. Para el permiso postnatal parental (art. 197 bis: 12 semanas completas o 18 a media jornada), <a href="/postnatal-parental">calcular postnatal parental</a>. Para el fuero maternal (art. 201: un año después del postnatal, excluido el parental), <a href="/fuero-maternal">calcular fuero maternal</a>. Para el permiso pagado del padre por nacimiento (art. 195: 5 días con goce), <a href="/permiso-paternidad">calcular permiso paternidad</a>. Para el permiso pagado por matrimonio o AUC (art. 207 bis: 5 días hábiles), <a href="/permiso-matrimonio">calcular permiso matrimonio</a>. Para el permiso pagado por fallecimiento de un familiar (art. 66: 10, 7 o 4 días), <a href="/permiso-fallecimiento">calcular permiso fallecimiento</a>. Para el reajuste IPC e interés si el empleador paga tarde remuneraciones (art. 63), <a href="/interes-mora">calcular interés por mora</a>. Para el valor de la hora de alimentación o lactancia (art. 206, tiempo remunerado hasta los 2 años), <a href="/hora-lactancia">calcular hora de lactancia</a>. Para colación y movilización del artículo 41, <a href="/colacion-movilizacion">calcular colación y movilización</a>. Para el viático de una comisión fuera del lugar habitual, <a href="/viatico">calcular viático</a>. Para las horas extras (art. 32, recargo 50 %), <a href="/horas-extras">calcular horas extras</a>. Para el tope gradual de jornada ordinaria (Ley 21.561, 42 h en 2026) y el valor hora DT, <a href="/jornada-40-horas">calcular jornada 40 horas</a>. Para el tope de 2/3 de una jornada parcial (art. 40 bis), el sueldo proporcional a horas y el feriado en días, <a href="/jornada-parcial">calcular jornada parcial</a>. Para el derecho a desconexión de al menos 12 horas continuas en teletrabajo o trabajo a distancia (Ley 21.220 / art. 152 quáter J), <a href="/teletrabajo">calcular teletrabajo</a>. Para las bandas horarias de cuidado familiar (Ley 21.561: anticipar o retrasar hasta 1 hora el inicio si es madre, padre o tiene el cuidado de un niño/a de hasta 12 años), <a href="/bandas-horarias">calcular bandas horarias</a>. Para el pacto 4×3 (Ley 21.561 art. 8° transitorio: 4 días de trabajo y 3 de descanso, tope 10 h/día), <a href="/pacto-4x3">calcular pacto 4×3</a>. Para el PHSC de un sistema excepcional de jornada y descansos (art. 38 incisos 7°–9° / DS N°48; requiere autorización DT), <a href="/jornada-excepcional">calcular jornada excepcional</a>. Para un ciclo de jornada bisemanal (art. 39: hasta 12 días continuos y mínimo 3 de descanso, promedio semanal frente al tope 42/40 h), <a href="/jornada-bisemanal">calcular jornada bisemanal</a>. Para compensar horas extras por días adicionales de feriado (art. 32 inc. 4° / Ley 21.561: 1,5 h de feriado por HE, tope 5 días hábiles), <a href="/compensacion-horas-extras">calcular compensación de horas extras</a>. Para estimar el pago cuando hay o no pacto escrito (arts. 31 y 32: recargo 50 % y tope orientativo de 2 h al día), <a href="/pacto-horas-extras">calcular pacto de horas extras</a>. Para los días de descanso compensatorio por domingo o festivo trabajado (art. 38), <a href="/descanso-compensatorio">calcular descanso compensatorio</a>. Para el pago de un feriado irrenunciable (Ley 19.973 + recargo 50 % art. 32), <a href="/feriado-irrenunciable">calcular pago feriado irrenunciable</a>. Para el séptimo día (art. 45), <a href="/semana-corrida">calcular semana corrida</a>. Para el aguinaldo de Fiestas Patrias (presupuesto, no obligación legal general), <a href="/aguinaldo">calcular aguinaldo</a>.</p>
      ${hubList(liq)}
      <h2>Finiquito</h2>
      <p>Causales, plazos del artículo 177, indemnizaciones y qué firmar. Para el finiquito completo use <a href="/finiquito">/finiquito</a>. La IAS sola (art. 163) está en <a href="/indemnizacion-anos-servicio">calcular indemnización por años de servicio</a>. Los años cumplidos, meses y días de fecha a fecha que alimentan la IAS y el feriado proporcional están en <a href="/antiguedad-laboral">calcular antigüedad laboral</a>. Si la remuneración tiene comisiones u otros variables, el promedio de los últimos tres meses (art. 172) está en <a href="/promedio-remuneraciones">calcular promedio de remuneraciones</a>. La indemnización por tiempo servido de un contrato por obra o faena (2,5 días/mes, art. 159 N°5) está en <a href="/obra-faena">calcular indemnización obra faena</a>. El tope de duración de un contrato a plazo fijo (12 o 24 meses) y si pasa a indefinido (art. 159 N°4) está en <a href="/contrato-plazo-fijo">calcular contrato a plazo fijo</a>. La remuneración remanente si el empleador termina ese plazo fijo antes del vencimiento, sin causal del art. 160, está en <a href="/termino-anticipado-plazo-fijo">calcular término anticipado plazo fijo</a>. La sustitutiva del aviso (arts. 161 y 162) está en <a href="/indemnizacion-aviso-previo">calcular indemnización por aviso previo</a>. Las remuneraciones por nulidad del despido (art. 162, cotizaciones impagas) están en <a href="/nulidad-despido">calcular nulidad del despido</a>. La indemnización especial de tutela laboral (art. 489: 6 a 11 meses) está en <a href="/tutela-laboral">calcular tutela laboral</a>. El recargo del art. 168 (despido injustificado, indebido o improcedente) está en <a href="/despido-injustificado">calcular despido injustificado</a>. Si el trabajador pone término por incumplimiento del empleador (art. 171), <a href="/autodespido">calcular autodespido</a>. El plazo para reclamar derechos laborales (art. 510) y los 60 días hábiles del art. 168 están en <a href="/prescripcion-laboral">calcular prescripción laboral</a>. Casa particular (fondo AFP 1,11 %, no IAS de 30 días) está en <a href="/finiquito-casa-particular">calcular finiquito casa particular</a>. Las fechas del feriado anual (art. 67) están en <a href="/feriado-anual">calcular feriado anual</a>. Los días extra del artículo 68 están en <a href="/feriado-progresivo">calcular feriado progresivo</a>.</p>
      ${hubList(fini)}
